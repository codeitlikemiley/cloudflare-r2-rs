...
fn trailing_slash_is_trimmed_from_the_endpoint() {
    ...
    .unwrap();
    // The stored endpoint keeps what the caller gave; the signed URL must not
    // end up with a doubled slash.
    assert!(client.endpoint().starts_with("https://acct."));
}
...
#[tokio::test]
async fn keys_that_escape_the_download_directory_are_refused() {
    let client = client();
    for key in ["", "../secrets.txt", "a/../../b.txt"] {
...
#[tokio::test]
async fn upload_file_reports_a_missing_local_file_as_io() {
    let err = client()
